...
export function listCheckpoints(): Checkpoint[] {
  ensureDir();
  try {
    const files = require("node:fs").readdirSync(CHECKPOINTS_DIR).filter((f: string) => f.endsWith(".json"));
    return files
      .map((f: string) => {
        try {
          return JSON.parse(readFileSync(join(CHECKPOINTS_DIR, f), "utf-8")) as Checkpoint;
        } catch {
          return null;
        }
      })
      .filter(Boolean)
      .sort((a: Checkpoint, b: Checkpoint) => b.timestamp.localeCompare(a.timestamp)) as Checkpoint[];
  } catch {
    return [];
  }
}
 ...
 */
export function restoreCheckpoint(id: string, cwd: string): {
  success: boolean;
  messages?: Message[];
  error?: string;
} {
 ...
 */
export function formatCheckpointsList(checkpoints: Checkpoint[]): string {
  if (checkpoints.length === 0) return "  Keine Checkpoints vorhanden.";
  return checkpoints
    .map(cp => {
