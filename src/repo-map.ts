...
function walkDir(dir: string, base: string, files: string[], maxDepth = 6, depth = 0): void {
  ...
  try {
    for (const entry of readdirSync(dir)) {
      ...
      try {
        ...
        if (stat.isDirectory()) walkDir(full, base, files, maxDepth, depth + 1);
        else if (stat.isFile() && stat.size < 500000) files.push(relative(base, full));
      } catch {}
    }
  } catch {}
}
...
export function getRepoStats(cwd: string): RepoStats {
  const files: string[] = [];
...
export function getRepoMap(cwd: string): FileSignature[] {
  const files: string[] = [];
  walkDir(cwd, cwd, files);
  ...
  for (const file of files) {
    ...
    if (signatures.length >= 200) break;
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (namedExp) exports.push(...namedExp[1].split(",").map(s => s.trim().split(/\s+as\s+/)[0].trim()).filter(Boolean));
        // Imports
        const impMatch = line.match(/(?:import|from)\s+["']([^"']+)["']/);
        if (impMatch) imports.push(impMatch[1]);
        // Python
        const pyImp = line.match(/^(?:import|from)\s+(\S+)/);
        if (ext === ".py" && pyImp) imports.push(pyImp[1]);
        // Python def/class
        const pyDef = line.match(/^(?:def|class)\s+(\w+)/);
        if (ext === ".py" && pyDef) exports.push(pyDef[1]);
      }
...
export function getRepoMapCompact(cwd: string): string {
...
export function generateOnboarding(cwd: string): string {
  ...
  try {
    const dirs = readdirSync(cwd).filter(d => {
      if (d.startsWith(".") || IGNORE_DIRS.has(d)) return false;
...
export function generateProjectScore(cwd: string): {
  quality: number; testCoverage: number; typeSafety: number;
  documentation: number; security: number; overall: number;
  quickWins: string[];
} {
  ...
  // Quality
  if (files.some(f => f.includes("eslint") || f.includes(".eslintrc"))) quality += 15;
  else quickWins.push("+15 Quality: ESLint konfigurieren");
  if (files.some(f => f.includes("prettier") || f.includes(".prettierrc"))) quality += 10;
  else quickWins.push("+10 Quality: Prettier hinzufuegen");
  ...
  if (testCoverage < 30) quickWins.push(`+${30 - testCoverage} Tests: Mehr Tests schreiben (${testFiles.length}/${srcFiles.length})`);

  // Type safety
  const tsFiles = files.filter(f => f.endsWith(".ts") || f.endsWith(".tsx"));
  const jsFiles = files.filter(f => f.endsWith(".js") || f.endsWith(".jsx"));
  if (tsFiles.length > jsFiles.length) typeSafety = 70;
  if (existsSync(join(cwd, "tsconfig.json"))) {
    typeSafety += 20;
    try {
      const tsconfig = readFileSync(join(cwd, "tsconfig.json"), "utf-8");
      if (tsconfig.includes('"strict": true') || tsconfig.includes('"strict":true')) typeSafety += 10;
    } catch {}
  }
  // Check for "any" usage
  let anyCount = 0;
  ...
  if (anyCount > 10) { typeSafety = Math.max(0, typeSafety - 20); quickWins.push(`+20 TypeSafety: ${anyCount}x "any" entfernen`); }

  // Documentation
  if (files.some(f => f === "README.md")) documentation += 30;
  else quickWins.push("+30 Docs: README.md erstellen");
  if (files.some(f => f.includes("CHANGELOG") || f.includes("changelog"))) documentation += 15;
  if (files.some(f => f.includes("CONTRIBUTING"))) documentation += 15;
  // Check for JSDoc
  let hasJsdoc = false;
  for (const f of tsFiles.slice(0, 20)) {
    try { if (readFileSync(join(cwd, f), "utf-8").includes("/**")) { hasJsdoc = true; break; } } catch {}
  }
  if (hasJsdoc) documentation += 20;
  else quickWins.push("+20 Docs: JSDoc zu public Funktionen");

  // Security
  if (files.some(f => f === ".env.example" || f === ".env.template")) security += 10;
  if (files.some(f => f === ".gitignore")) security += 10;
  // Check for hardcoded secrets
  let secretsFound = 0;
  for (const f of files.filter(ff => CODE_EXTS.has(extname(ff))).slice(0, 30)) {
    try {
      const content = readFileSync(join(cwd, f), "utf-8");
      if (content.match(/(?:api[_-]?key|secret|password|token)\s*[:=]\s*["'][^"']{8,}/i)) secretsFound++;
    } catch {}
  }
  if (secretsFound > 0) { security -= 30; quickWins.push(`+30 Security: ${secretsFound} Dateien mit moeglichen Secrets`); }
...
export function generateCodeRoast(cwd: string): string {
  const roasts: string[] = [];
  const files: string[] = [];
  walkDir(cwd, cwd, files);
  let score = 7;
  ...
  for (const f of files.filter(ff => ff.endsWith(".ts") || ff.endsWith(".tsx")).slice(0, 50)) {
    try { anyCount += (readFileSync(join(cwd, f), "utf-8").match(/:\s*any\b/g) || []).length; } catch {}
  }
  if (anyCount > 0) { roasts.push(`${anyCount}x "any" gefunden. TypeScript ist kein JavaScript mit Extra-Schritten — es hat einen Sinn.`); score -= 1; }

  // console.log in src
  let consoleLogs = 0;
  for (const f of files.filter(ff => CODE_EXTS.has(extname(ff)) && !ff.includes("test")).slice(0, 50)) {
    try { consoleLogs += (readFileSync(join(cwd, f), "utf-8").match(/console\.log/g) || []).length; } catch {}
  }
  if (consoleLogs > 5) { roasts.push(`${consoleLogs}x console.log im Produktionscode. Das ist kein Debugging, das ist Hoffnung.`); score -= 1; }

  // No tests
  const testFiles = files.filter(f => f.includes(".test.") || f.includes(".spec."));
  if (testFiles.length === 0) { roasts.push("Keine Tests gefunden. Mutig. Ihr testet in Production wie echte Helden."); score -= 2; }

  // TODOs
  let todoCount = 0;
  for (const f of files.filter(ff => CODE_EXTS.has(extname(ff))).slice(0, 50)) {
    try { todoCount += (readFileSync(join(cwd, f), "utf-8").match(/\/\/\s*TODO/gi) || []).length; } catch {}
  }
  if (todoCount > 3) { roasts.push(`${todoCount} TODOs gefunden. Das T steht fuer "Tomorrow". Oder "Traumwelt".`); score -= 0.5; }

  // Large files
  for (const f of files.filter(ff => CODE_EXTS.has(extname(ff))).slice(0, 100)) {
    try {
      const lines = readFileSync(join(cwd, f), "utf-8").split("\n").length;
      if (lines > 500) { roasts.push(`${f} hat ${lines} Zeilen. Das ist kein File, das ist ein Roman. Refactoring waere nett.`); score -= 0.5; break; }
    } catch {}
  }

  // .env in git
  if (files.includes(".env")) { roasts.push(".env Datei im Projekt. Hoffentlich nicht im Git. Bitte .gitignore pruefen."); score -= 1; }

  // No .gitignore
  if (!files.includes(".gitignore")) { roasts.push("Keine .gitignore? Das ist wie ein Haus ohne Tuer. node_modules sagt Danke."); score -= 1; }

  // No README
  if (!files.some(f => f.toLowerCase() === "readme.md")) { roasts.push("Kein README.md. Wie soll jemand wissen was dieses Projekt macht? Hellsehen?"); score -= 0.5; }
  ...
  for (const roast of roasts) output += `  - ${roast}\n`;
  if (roasts.length === 0) output += "  Hmm... eigentlich sieht das recht gut aus. Respekt.\n";
