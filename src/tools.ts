...
export async function executeToolCalls(response: string, cwd: string): Promise<{ results: ToolResult[]; cleanResponse: string }> {
  ...
  if (results.length === 0) {
    ...
    while ((htmlMatch = htmlBlockRegex.exec(response)) !== null) {
      ...
      if (!looksLikeHTML || looksLikeReact) continue;
      ...
      try {
        ...
        results.push({ tool: "auto-html", result: `Fehler: ${(e as Error).message}`, success: false });
      }
      cleanResponse = cleanResponse.replace(fullBlock, "");
    }
  }
...
export function executeNativeToolCall(
  ...
  switch (name) {
    ...
    case "ls":
      return listDir((args.dirPath as string) || ".", cwd);
    case "git":
...
export async function executeNativeToolCallsParallel(
  ...
  const safeBatch: { index: number; call: NativeToolCall }[] = [];
    ...
    if (SAFE_TOOLS.has(call.name)) {
      ...
      if (safeBatch.length > 0) {
        const safeResults = await Promise.all(
          safeBatch.map(({ call: c }) => Promise.resolve(executeNativeToolCall(c.name, c.arguments, cwd)))
        );
