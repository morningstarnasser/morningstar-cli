 ...
 */
function resolveImports(content: string, baseDir: string, visited: Set<string>): string {
  return content.replace(/@import\s+([^\n]+)/g, (_match, importPath: string) => {
    ...
    try {
      ...
      const { content: importedContent } = parseFrontmatter<Record<string, unknown>>(imported);
 ...
 */
export function getApplicableRules(cwd: string, filePath?: string): Rule[] {
...
export function createRule(
  id: string,
  content: string,
  options?: { description?: string; pathPattern?: string; priority?: number; global?: boolean },
  cwd?: string
  ...
  if (options?.pathPattern) frontmatter.pathPattern = options.pathPattern;
  if (options?.priority !== undefined) frontmatter.priority = options.priority;
...
export function buildRulesPrompt(cwd: string): string {
  ...
  for (const rule of rules) {
    if (rule.id === "MORNINGSTAR") continue; // Already injected via project memory
...
export function formatRulesList(rules: Rule[]): string {
  if (rules.length === 0) return "  Keine Rules geladen.";
  return rules
    .map(r => {
