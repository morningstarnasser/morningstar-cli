...
export function createFileWatcher(
  ...
  function shouldWatch(filePath: string): boolean {
    ...
    if (!cfg.extensions.has(ext)) return false;
  ...
  function handleChange(eventType: string, filename: string | null, dir: string): void {
    ...
    pending.set(key, setTimeout(() => {
      ...
      let type: FileChangeEvent["type"] = "modify";
      try {
 ...
 */
export function formatFileChanges(events: FileChangeEvent[]): string {
  if (events.length === 0) return "  Keine Aenderungen erkannt.";
 ...
 */
export function summarizeChanges(events: FileChangeEvent[]): string {
  ...
  const parts: string[] = [];
  if (created.length > 0) parts.push(`${created.length} erstellt (${created.map(e => e.relativePath).join(", ")})`);
  if (modified.length > 0) parts.push(`${modified.length} geaendert (${modified.map(e => e.relativePath).join(", ")})`);
  if (deleted.length > 0) parts.push(`${deleted.length} geloescht (${deleted.map(e => e.relativePath).join(", ")})`);
 ...
 */
export function detectWatchDirs(cwd: string): string[] {
  ...
  for (const dir of candidates) {
    ...
    if (existsSync(fullPath)) {
      try {
        if (statSync(fullPath).isDirectory()) found.push(dir);
      } catch {}
    }
  }
