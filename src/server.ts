...
export async function startServer(config: ServerConfig): Promise<{ close: () => void; port: number; url: string }> {
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      // ─── POST /v1/chat/completions ─────────────────
        ...
        const body = JSON.parse(await readBody(req)) as { messages?: Message[]; stream?: boolean; temperature?: number; max_tokens?: number; model?: string };
        if (!body.messages?.length) { sendError(res, 400, "messages required"); return; }

        const cfg: CLIConfig = { ...config.cliConfig };
        if (body.temperature !== undefined) cfg.temperature = body.temperature;
        if (body.max_tokens !== undefined) cfg.maxTokens = body.max_tokens;
        ...
        if (body.stream) {
          ...
          let content = "";
          for await (const chunk of streamChat(body.messages, cfg)) { if (chunk.type === "content") content += chunk.text; }
      ...
      // ─── POST /v1/vision/analyze ───────────────────
        ...
        const body = JSON.parse(await readBody(req)) as { image?: string; prompt?: string; model?: string };
        if (!body.image) { sendError(res, 400, "image required (base64 or file path)"); return; }
      ...
      // ─── POST /v1/images/generate ──────────────────
        ...
        if (!(await isSetupComplete())) { sendError(res, 503, "Image generation not set up. Run /imagine setup first."); return; }
        const body = JSON.parse(await readBody(req)) as { prompt?: string; model?: string; steps?: number; width?: number; height?: number; negative_prompt?: string; seed?: number };
        if (!body.prompt) { sendError(res, 400, "prompt required"); return; }
