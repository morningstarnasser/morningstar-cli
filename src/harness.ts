...
const EXPECTED_FILES: ReadonlyArray<{ name: string; isJson: boolean }> = [
...
function safeReadJson(filePath: string): { valid: boolean; data: unknown; error?: string } {
  try {
...
function safeStat(filePath: string): { size: number; isDir: boolean } | null {
  try {
...
function validateFrontmatter(filePath: string): { valid: boolean; error?: string } {
  try {
    const raw = readFileSync(filePath, "utf-8");
...
function collectMdFilePaths(dir: string): string[] {
  const paths: string[] = [];
...
function checkConfigFile(): HarnessCheck {
...
function checkSettingsFile(): HarnessCheck {
...
function checkAgentsDir(): HarnessCheck {
...
function checkSkillsDir(): HarnessCheck {
...
function checkCommandsDir(): HarnessCheck {
...
function checkRulesDir(): HarnessCheck[] {
...
function checkMemoryFile(): HarnessCheck {
...
function checkLearningDir(): HarnessCheck {
...
function checkDuplicateAgentIds(): HarnessCheck {
...
function checkBrokenFrontmatter(category: CheckCategory, dir: string, label: string): HarnessCheck[] {
  const checks: HarnessCheck[] = [];
...
function auditDuplicateAgents(): HarnessCheck {
  const dir = join(BASE_DIR, "agents");
  if (!existsSync(dir)) {
    return {
  ...
  for (const file of mdFiles) {
    try {
      ...
      const { frontmatter } = parseFrontmatter<{ name?: string }>(raw);
      const name = ((frontmatter.name as string) || basename(file, ".md")).toLowerCase();
...
function auditOrphanedSkills(): HarnessCheck {
  const dir = join(BASE_DIR, "skills");
  if (!existsSync(dir)) {
    return {
  ...
  for (const filePath of mdFiles) {
    try {
      const raw = readFileSync(filePath, "utf-8");
      const { frontmatter } = parseFrontmatter<{ triggers?: string[]; description?: string }>(raw);
...
function auditOrphanedRuleDirs(): HarnessCheck {
  const dir = join(BASE_DIR, "rules");
  if (!existsSync(dir)) {
    return {
...
function auditContentCounts(): HarnessCheck {
...
function auditLargeFiles(): HarnessCheck {
  const large: Array<{ path: string; sizeKb: number }> = [];
...
function auditMcpCatalog(): HarnessCheck {
...
function auditObservationsSize(): HarnessCheck {
 ...
 */
export function runDoctor(): Promise<HarnessCheck[]> {
  const checks: HarnessCheck[] = [];
  ...
  try {
    ...
    const contentDirs: Array<{ category: CheckCategory; dir: string; label: string }> = [
    ...
    checks.push({
      ...
      category: "config",
      status: "fail",
      message: `Unexpected error during doctor checks: ${(err as Error).message}`,
    });
  }
 ...
 */
export function runAudit(): Promise<HarnessCheck[]> {
  const checks: HarnessCheck[] = [];
  ...
  try {
    ...
    checks.push({
      ...
      category: "config",
      status: "fail",
      message: `Unexpected error during audit checks: ${(err as Error).message}`,
    });
  }
 ...
 */
export function runRepair(checks: ReadonlyArray<HarnessCheck>): RepairResult {
  let fixed = 0;
  let remaining = 0;
...
function applyFix(fix: string): boolean {
  try {
    ...
    const jsonFixMap: Record<string, { path: string; defaultContent: string }> = {
...
function removeDuplicateAgents(): boolean {
  const dir = join(BASE_DIR, "agents");
  if (!existsSync(dir)) return false;

  const mdFiles = safeReaddirSync(dir).filter(f => f.endsWith(".md"));
  const nameMap = new Map<string, Array<{ file: string; mtime: number }>>();
...
function groupByCategory(checks: ReadonlyArray<HarnessCheck>): Map<CheckCategory, HarnessCheck[]> {
  const groups = new Map<CheckCategory, HarnessCheck[]>();
...
function formatSummary(checks: ReadonlyArray<HarnessCheck>): string {
 ...
 */
export function formatDoctorReport(checks: ReadonlyArray<HarnessCheck>): string {
  const lines: string[] = [
    "",
 ...
 */
export function formatAuditReport(checks: ReadonlyArray<HarnessCheck>): string {
  const lines: string[] = [
    "",
