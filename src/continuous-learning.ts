import { existsSync, readFileSync, writeFileSync, mkdirSync, appendFileSync, readdirSync, unlinkSync, renameSync, statSync } from "node:fs";
...
const VALID_DOMAINS = ["code-style", "testing", "git", "debugging", "workflow", "general"] as const;
...
function ensureLearningDirs(): void {
...
function projectObservationsDir(projectId: string): string {
  return join(PROJECTS_DIR, projectId);
}

function projectObservationsFile(projectId: string): string {
  return join(projectObservationsDir(projectId), "observations.jsonl");
}

function instinctFilePath(instinct: Instinct): string {
...
function rotateObservationsFile(filePath: string): void {
  try {
    if (!existsSync(filePath)) return;
    const lineCount = countLines(filePath);
    if (lineCount <= MAX_OBSERVATIONS_PER_FILE) return;
...
function readJsonlFile(filePath: string, limit?: number): Observation[] {
  try {
    if (!existsSync(filePath)) return [];
    const content = readFileSync(filePath, "utf-8");
    ...
    for (let i = startIndex; i < lines.length; i++) {
      try {
        results.push(JSON.parse(lines[i]) as Observation);
      } catch {
...
function loadInstinctsFromDir(dir: string): Instinct[] {
  try {
    ...
    for (const file of files) {
      const instinct = safeReadJsonFile<Instinct>(join(dir, file));
 ...
 */
export function getObservations(projectId?: string, limit: number = 100): Observation[] {
 ...
 */
export function loadInstincts(projectId?: string): Instinct[] {
...
export function saveInstinct(instinct: Instinct): void {
  ...
  const validated: Instinct = {
    ...
    domain: VALID_DOMAINS.includes(instinct.domain as Domain) ? instinct.domain : "general",
 ...
 */
export function updateConfidence(instinctId: string, delta: number): Instinct | null {
  const filePath = findInstinctFile(instinctId);
  if (!filePath) return null;

  const instinct = safeReadJsonFile<Instinct>(filePath);
 ...
 */
export function promoteInstinct(instinctId: string): void {
  const sourcePath = findInstinctFile(instinctId);
  if (!sourcePath) return;

  const instinct = safeReadJsonFile<Instinct>(sourcePath);
  if (!instinct) return;

  // Only promote if currently project-scoped
  if (instinct.scope !== "project") return;
 ...
 */
export function importInstincts(json: string): { imported: number; errors: string[] } {
  const errors: string[] = [];
  let imported = 0;
  ...
  for (let i = 0; i < instinctArray.length; i++) {
    try {
      const raw = instinctArray[i] as Record<string, unknown>;
      ...
      const instinct: Instinct = {
        ...
        domain: VALID_DOMAINS.includes(raw.domain as Domain) ? (raw.domain as Domain) : "general",
        scope: "global", // Imported instincts are always global
        evidence: Array.isArray(raw.evidence)
          ? (raw.evidence as string[]).slice(0, 20).map(e => String(e).slice(0, 200))
          : [],
 ...
 */
export function getInstinctsPromptAddition(projectId?: string): string {
 ...
 */
export function formatInstinctsStatus(projectId?: string): string {
...
function getObservationCount(filePath: string): number {
  return countLines(filePath);
}

function isInherited(instinctId: string): boolean {
  const inheritedPath = join(INSTINCTS_INHERITED_DIR, `${instinctId}.json`);
