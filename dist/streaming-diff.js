 ...
 */
export function createStreamingDiffState() {
    return {
        filePath: null,
        originalLines: [],
        oldString: "",
 ...
 */
export function parsePartialEditCall(text) {
...
function parseJsonEditArgs(text) {
...
function parseSimpleEditArgs(text, filePath) {
 ...
 */
export function updateStreamingDiff(state, streamedText, cwd) {
    ...
    if (parsed.newString !== undefined) {
        ...
        // Compute the diff
        newState.diffLines = computeStreamingDiff(newState.oldString, newState.newString, newState.startLine, newState.filePath || "");
    }
    return newState;
}
/**
 ...
 */
function findSubsequence(original, search) {
    if (search.length === 0)
        return -1;
 ...
 */
export function computeStreamingDiff(oldStr, newStr, startLine, filePath) {
    const lines = [];
 ...
 */
export function formatStreamingDiff(state) {
 ...
 */
export function hasEditOperation(text) {
