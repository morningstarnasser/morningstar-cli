...
import { detectProvider, getProviderBaseUrl, listProviders, getModelDisplayName } from "./providers.js";
import { getPermissionMode, setPermissionMode } from "./permissions.js";
import { initProjectSettings, projectSettingsExist, getProjectSettingsPath, getGlobalSettingsPath } from "./settings.js";
...
export function App({ config: initialConfig, ctx, chatOnly, skipPermissions, baseSystemPrompt, sessionStart, getStoredApiKey, storeApiKey, saveConfig }) {
    const { exit } = useApp();
    const { primary, success: successColor, error: errorColor, warning, info, dim, accent, star } = useTheme();
    // ── State ──
    const [config, setConfig] = useState(initialConfig);
    const [messages, setMessages] = useState([{ role: "system", content: getFullSystemPrompt() }]);
    const [activeAgent, setActiveAgent] = useState(null);
    const [planMode, setPlanMode] = useState(false);
    const [thinkMode, setThinkMode] = useState(false);
    const [isProcessing, setIsProcessing] = useState(false);
    const [output, setOutput] = useState([{ id: 0, type: "banner" }]);
    const [slashCommands] = useState(() => buildSlashCommands());
    ...
    const processInput = useCallback(async (input) => {
        ...
        try {
            ...
            if (!chatOnly) {
                ...
                try {
                    toolResults = await executeToolCalls(fullResponse, ctx.cwd);
                }
                catch { }
                if (toolResults && toolResults.results.length > 0) {
                    ...
                    while (depth < 5 && currentResponse && !signal.aborted) {
                        let nested = null;
                        try {
                            nested = await executeToolCalls(currentResponse, ctx.cwd);
                        }
                        catch {
                            break;
                        }
