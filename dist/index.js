...
async function interactiveModelSelect() {
    ...
    if (prov !== "ollama" && !config.apiKey) {
        ...
        if (envKey)
            console.log(d(`  Oder: export ${envKey}=DEIN_KEY\n`));
    }
}
// ─── Git Helpers ─────────────────────────────────────────
function runGit(cmd) {
    ...
    switch (cmd) {
        ...
        case "/permissions": {
            if (arg && ["auto", "ask", "strict"].includes(arg.toLowerCase())) {
                ...
                const labels = { auto: "Automatisch (alle Tools)", ask: "Nachfragen (bei Schreibzugriff)", strict: "Strikt (bei jedem Tool)" };
        ...
        case "/score": {
            ...
            if (score.quickWins.length > 0) {
                ...
                for (const win of score.quickWins)
                    console.log(chalk.gray(`    ${win}`));
            }
            console.log();
            return true;
        }
        ...
        case "/map": {
            ...
            if (map.length === 0) {
                console.log(chalk.gray("  Keine Code-Dateien gefunden.\n"));
                return true;
            }
            for (const file of map.slice(0, 50)) {
                console.log(chalk.hex(t().accent)(`  ${file.path}`) + chalk.gray(` (${file.lines} Zeilen)`));
                if (file.exports.length > 0)
                    console.log(chalk.hex(t().success)(`    exports: ${file.exports.join(", ")}`));
                if (file.imports.length > 0)
                    console.log(chalk.hex(t().dim)(`    imports: ${file.imports.join(", ")}`));
            }
            if (map.length > 50)
