...
function scanProject(cwd, maxFiles = 80) {
    const files = [];
    function walk(dir, depth) {
        if (depth > 6 || files.length >= maxFiles)
            return;
        let entries;
        try {
...
export function buildDepGraph(cwd) {
...
function assignLayers(graph) {
    ...
    function getLayer(path, visited) {
        if (layers.has(path))
...
export function renderDepGraphAscii(graph, maxWidth = 80) {
        ...
        for (const row of rows) {
            ...
            if (gi < layerGroups.length - 1) {
                ...
                for (let i = 0; i < row.length; i++) {
                    ...
                    const hasChildren = node.imports.some(imp => layerGroups[gi + 1]?.some(n => n.path === imp));
