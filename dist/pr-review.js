...
function parsePRNumber(ref) {
    ...
    if (pullMatch)
        return parseInt(pullMatch[1], 10);
...
// ─── 1. Fetch PR Data ───────────────────────────────────
export function fetchPRData(prUrl, cwd) {
    if (!ghInstalled())
        throw new Error("GitHub CLI (gh) nicht installiert. Installation: https://cli.github.com/");
    if (!isGitRepo(cwd))
        throw new Error(`"${cwd}" ist kein Git-Repository.`);
    ...
    try {
        ...
        throw new Error("Ungueltige JSON-Antwort von gh CLI.");
    }
    ...
    let diff;
    try {
        diff = runGh(`pr diff ${prNumber}`, cwd);
    }
    catch (e) {
        throw new Error(`Fehler beim Abrufen des PR-Diffs: ${e.message.slice(0, 200)}`);
    }
    return { prData, diff };
}
// ─── 2. Analyze Diff ────────────────────────────────────
export function analyzePRDiff(diff) {
    const chunks = [];
    const filesByType = {};
    const largeDiffs = [];
    let hasTests = false;
    ...
    for (let i = 0; i < headers.length; i++) {
        ...
        if (TEST_PATTERNS.some(p => p.test(filePath)))
            hasTests = true;
        if (CONFIG_PATTERNS.some(p => p.test(filePath) || p.test(basename(filePath))))
...
export function generateReviewPrompt(prData, analysis) {
    const parts = [];
    ...
    if (!analysis.hasTests)
        flags.push("KEINE Tests vorhanden");
    if (analysis.hasTests)
        flags.push("Tests vorhanden");
    if (analysis.hasConfigChanges)
        flags.push("Konfigurationsaenderungen enthalten");
    if (analysis.largeDiffs.length > 0)
        flags.push(`Grosse Diffs: ${analysis.largeDiffs.join(", ")}`);
    if (flags.length > 0) {
        parts.push(`\n## Hinweise\n`);
    ...
    parts.push("\n## Aufgabe\n");
    parts.push("Fuehre ein Code Review durch. Analysiere auf:\n" +
...
export function parseReviewComments(review) {
    const comments = [];
...
export function postReviewComments(prNumber, comments, cwd) {
    if (!ghInstalled())
        throw new Error("GitHub CLI (gh) nicht installiert.");
    ...
    try {
        ...
        throw new Error(`HEAD-Commit fuer PR #${prNumber} nicht ermittelbar: ${e.message.slice(0, 100)}`);
    }
    ...
    try {
        ...
        errors.push(`Review-Post fehlgeschlagen: ${e.message.slice(0, 150)}`);
        ...
        try {
            ...
            errors.push(`Fallback-Kommentar fehlgeschlagen: ${e2.message.slice(0, 100)}`);
        }
    }
