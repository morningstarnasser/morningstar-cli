...
export async function startImageServer(onProgress) {
    ...
    if (!ready)
        throw new Error("Server konnte nicht gestartet werden. Evtl. Model-Download noch nicht fertig.");
    onProgress?.("Image Server bereit!");
}
export async function stopImageServer() {
    if (existsSync(PID_FILE)) {
        try {
            const pid = parseInt(readFileSync(PID_FILE, "utf-8").trim());
...
async function generateWithGemini(prompt, outPath, modelVariant = "gemini") {
