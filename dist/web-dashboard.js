...
// ─── HTML Template ───────────────────────────────────────
function generateDashboardHTML(config, costs, stats, messages, sessionStart) {
 ...
 */
export async function startDashboard(dashConfig) {
    const server = createServer((req, res) => {
        ...
        if (url === "/" || url === "/dashboard") {
            ...
            const html = generateDashboardHTML(dashConfig.cliConfig, costs, toolStats, dashConfig.getMessages(), dashConfig.getSessionStart());
 ...
 */
export function formatDashboardStatus(state) {
    if (!state.isRunning)
        return "  Dashboard: Nicht aktiv";
    return [
        `  Dashboard aktiv`,
        `  URL:  ${state.url}`,
        `  Port: ${state.port}`,
